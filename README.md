...
**XGBoost** (Extreme Gradient Boosting) se ha seleccionado como algoritmo principal por cuatro razones: es el estándar de facto para datos tabulares (las soluciones ganadoras de la competición Rossmann original lo utilizaron), se entrena en segundos sobre instancias económicas, expone nativamente la importancia de cada feature, y se ajusta perfectamente a las restricciones del AWS Academy Learner Lab.
...
- **El feature engineering aporta más que el cambio de algoritmo:** la transición de XGBoost base a XGBoost+FE reduce RMSPE en un 9.5%, validando el principio de *better data beats better algorithms*.
